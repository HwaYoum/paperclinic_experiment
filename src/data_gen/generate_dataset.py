...
def extract_pdf_context(pdf_path: str, max_pages: int = 5) -> str:
...
def get_distractor_sentences() -> List[str]:
    ...
    try:
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            reader = csv.reader(f)
            for row in reader:
                if row: sentences.append(row[0].strip())
...
def generate_validated_noisy_essay(
    ...
    for attempt in range(max_retries):
        ...
        if abs_diff <= 0.7: # Relaxed tolerance
            return noisy_text
        ...
        current_noise_param = max(0.5, min(5.0, current_noise_param))
