...
def get_gemini_response(prompt: str, model_name: str = "gpt-5-nano") -> str:
    try:
...
def evaluate_essay(text: str, target_trait: str) -> Dict[str, float]: ## 에세이 평가 / 이거 설마 정수가 아니라 실수로 나오나?
    ...
    response_text = get_gemini_response(prompt, model_name="gpt-5-nano")
...
def main():
    ...
    for pdf_path in pdf_files:
        ...
        for q_idx, question in enumerate(ESSAY_QUESTIONS):
            ...
            gold_text = get_gemini_response(prompt_gold, model_name="gpt-5-mini")
            if not gold_text:
